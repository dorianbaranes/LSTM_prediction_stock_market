...
class Model():
	...
	def build_model(self, configs,neurons_lstm,layer_lstm):
		...
		for layer in configs['model']['layers']:
			...
			if layer['type'] == 'lstm':
				self.model.add(LSTM(neurons_lstm, input_shape=(input_timesteps, input_dim), return_sequences=return_seq))
	...
	def train_generator(self, data_gen, epochs, batch_size, steps_per_epoch, save_dir, file_name_prefix):
		print('[Model] Training Started')
		print('[Model] %s epochs, %s batch size, %s batches per epoch %s lstm layers %s neurons per layer' % (epochs, batch_size, steps_per_epoch, self.lstm_layers, self.neurons_per_lstm_layer))
