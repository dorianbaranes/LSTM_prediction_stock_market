...
def plot_results_multiple(predicted_data, true_data, prices,  prediction_len):
    ...
    for val in range(int(len(prices)/10)+1):
        ax1.axvline(val*10, color='gray', linestyle='--', linewidth=0.5)
        ax2.axvline(val*10, color='gray', linestyle='--', linewidth=0.5)
...
def calculate_trend(test_data, forecast):
    ...
    for i, window in enumerate(forecast):
        ...
        test_trend_up = True if test_data[i * len_window] < test_data[i * len_window + len_window] else False

        if forecast_trend_up == test_trend_up:
            score += 1

    score_trend = (float(score / nb_predictions)) * 100.
...
def train_model(configs, model, data, archi_model):
    ...
    for batch_size in range(min_batch_size,max_batch_size+1,step_batch_size):
        ...
        steps_per_epoch = math.ceil((data.len_train - configs['data']['sequence_length']) / batch_size)
        ...
        for epoch in range(min_epoch , max_epoch + 1, step_epoch):
            ...
            models_properties[file_name_prefix] = {**{'RMSE': test_rmse, 'score_trend': score_trend, 'nb_predictions': nb_predictions, 'model':file_name_prefix+ '_model.h5'}, **archi_model}
...
def main():
    ...
    model_properties_path = os.path.join(configs['model']['save_dir'],f"{dt.datetime.now().strftime('%d%m%Y_%H%M%S')}_model_properties.json")
    ...
    with Timer() as timer:
        for lstm_layer in range(min_lstm_layer, max_lstm_layer+1, step_lstm_layer):
            for neurons in range(min_neurons_layer,max_neurons_layer+1, step_neurons_layer):
                archi_model = {'neurons_per_lstm_layer':neurons, 'nb_lstm_layer': lstm_layer}
                model = Model()
...
def check_model(filename):
    ...
    plot_results_multiple(predictions, y_test, prices, configs['data']['prediction_length'])
...
def sort_model_RMSE(filename):
    ...
    path_sort_models = os.path.join(configs['model']['save_dir'], 'sorted_RMSE_model.json')
...
def sort_model_trend(filename):
    ...
    # Write the sorted list to a JSON file
    path_sort_models = os.path.join(configs['model']['save_dir'], 'sorted_score_trend_model.json')
    with open(path_sort_models, 'w') as f:
        json.dump(sorted_list, f, indent=4)
...
if __name__ == '__main__':
    #main()
    check_model('27052023_234035_epc_2_batch_90_model.h5')
    sort_model_RMSE('27052023_222416_model_properties.json')
    sort_model_trend('27052023_222416_model_properties.json')